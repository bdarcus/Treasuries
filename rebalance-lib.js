...
export function runRebalance({
	...
	for (let i = holdings.length - 1; i >= 0; i--) {
		...
		const baseCpi = bond?.baseCpi ?? refCPI;
		const indexRatio = refCPI / baseCpi;
		if (!outputLaterMaturityInterest[h.year])
			outputLaterMaturityInterest[h.year] = 0;
	...
	const afterLowerWeight =
		totalExcessCost > 0 ? lowerExcessCost / totalExcessCost : null;
	const afterUpperWeight =
		totalExcessCost > 0 ? upperExcessCost / totalExcessCost : null;
