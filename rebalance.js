...
function parseArgs() {
	...
	for (let i = 0; i < args.length; i++) {
		if (args[i] === "--dara") {
			dara = parseFloat(args[++i]);
			continue;
		}
		if (args[i] === "--method") {
			const m = args[++i];
			method = m[0].toUpperCase() + m.slice(1).toLowerCase();
			continue;
		}
		holdingsFile = args[i];
	}
...
async function promptInputs(holdingsFile, dara, method) {
	...
	if (!holdingsFile) {
		holdingsFile = (
			await ask("Holdings CSV path [e.g. data/holdings.csv]: ")
		).trim();
	}
	if (dara === null) {
		const ans = (
			await ask("DARA (leave blank to infer from holdings): ")
		).trim();
		dara = ans ? parseFloat(ans) : null;
	}
	if (method === null) {
		const ans = (
			await ask("Rebalance method — Gap or Full [default: Gap]: ")
		).trim();
...
async function fetchSettlementCpi(dateStr) {
	...
	if (prior.length === 0)
		throw new Error(`No settlement CPI data on or before ${dateStr}`);
...
function buildTipsMap(baseCpiRows, priceRows, settleDateStr) {
	...
	for (const r of priceRows) {
		...
		if (price) {
			entry.price = price;
			entry.yield = yieldFromPrice(
				price,
				entry.coupon,
				settleDateStr,
				toDateStr(entry.maturity),
			);
		}
	}
	return map;
...
async function main() {
	...
	({ holdingsFile, dara, method } = await promptInputs(
		holdingsFile,
		dara,
		method,
	));
	...
	// ── Load holdings ──
	const holdingsRaw = fs
		.readFileSync(holdingsFile, "utf8")
		.trim()
		.split("\n")
		.filter((l) => l.trim())
		.map((line) => {
	...
	for (let attempt = 0; attempt < 5; attempt++) {
		...
		console.error(
			`No data for ${toDateStr(priceDate)}, trying previous weekday...`,
		);
	...
	for (const h of holdingsRaw) {
		...
		if (!bond) {
			console.error(
				`Warning: CUSIP ${h.cusip} not found in TIPS data — skipping`,
			);
			continue;
		}
		holdings.push({
	...
	for (const year of allYearsSorted) {
		...
		for (const holding of yearInfo[year].holdings) {
			...
			araLaterMaturityInterestByYear[year] +=
				holding.qty * adjustedAnnualInterest;
		}
	...
	console.error(
		`DARA: ${DARA.toFixed(2)}${dara === null ? " (inferred)" : ""}`,
	);
		...
		if (yh.length === 1) {
			tFYQty = Math.round(
				(DARA - laterMatIntBefore) /
					calculatePIPerBond(bracketCUSIP, bracketMaturity, refCPI, tipsMap),
			);
		} else {
			let nonPI = 0;
			for (const h of yh) {
				if (h.cusip !== bracketCUSIP)
					nonPI +=
						h.qty * calculatePIPerBond(h.cusip, h.maturity, refCPI, tipsMap);
			}
			tFYQty = Math.round(
	...
	if (isFullMode) {
		rebalYearSet = new Set(
			Object.keys(yearInfo)
				.map(Number)
				.filter(
					(y) =>
						y >= firstYear &&
						y <= lastYear &&
						!bracketYearSet.has(y) &&
						!gapYearSet.has(y),
				),
		);
	} else {
			...
			if (yi.holdings.length === 1) {
				targetFYQty = Math.round(
					(DARA - rebuildLaterMatInt) /
						calculatePIPerBond(targetCUSIP, targetMaturity, refCPI, tipsMap),
				);
			} else {
				let nonTargetPI = 0;
				for (const h of yi.holdings) {
	...
	for (const year of allYearsSorted) {
		...
		for (const h of yearInfo[year].holdings) {
			...
			const qtyARA =
				...
				h.cusip === buySellTargets[year]?.targetCUSIP
					? bracketTargetFYQtyBefore[year]
					: h.qty;
	...
	for (const year of allYearsSorted) {
		...
		for (const h of yearInfo[year].holdings) {
			...
			const qtyARA =
				bt && h.cusip === bt.targetCUSIP
					? bt.isBracket
						? bt.targetFYQty
						: bt.postRebalQty
					: postRebalQtyMap[h.cusip];
			yearP += qtyARA * 1000 * ir;
			const adjInt = 1000 * ir * (bond?.coupon ?? 0);
			yearI += qtyARA * (h.maturity.getMonth() + 1 < 7 ? adjInt * 0.5 : adjInt);
		}
	...
	for (let i = holdings.length - 1; i >= 0; i--) {
		...
		let sumLaterInt = 0;
		for (const yStr in outLaterInt)
			if (parseInt(yStr) > h.year) sumLaterInt += outLaterInt[yStr];
		...
		if (isLast) {
			...
			for (const hy of yearInfo[h.year].holdings) {
				...
				const adjInt = 1000 * ir * (bond?.coupon ?? 0);
				yearI +=
					hy.qty * (hy.maturity.getMonth() + 1 < 7 ? adjInt * 0.5 : adjInt);
				yearCost += hy.qty * (((bond?.price ?? 0) / 100) * ir * 1000);
			}
			fy = h.year;
			pFY = yearP;
			iFY = yearI + sumLaterInt;
			aFY = pFY + iFY;
			cFY = yearCost;
			aB = beforeARAByYear[h.year];
			amdB = aB - DARA;
			aA = postARAByYear[h.year];
			amdA = aA - DARA;
		}
		const bt = buySellTargets[h.year];
		if (bt && h.cusip === bt.targetCUSIP) {
			tQ = bt.targetQty;
			qD = bt.qtyDelta;
			tC = bt.targetCost;
			cD = bt.costDelta;
			if (bt.isBracket) {
				eB = bt.currentExcessCost;
				eA = (bt.postRebalQty - bt.targetFYQty) * bt.costPerBond;
			}
		}
		...
		outLaterInt[h.year] +=
			h.qty * 1000 * (refCPI / (bond?.baseCpi ?? refCPI)) * (bond?.coupon ?? 0);
	...
	const fmtI = (n) =>
		typeof n === "number" ? Math.round(n).toLocaleString() : String(n);
	const sign = (n) =>
		typeof n === "number" && n > 0
			? "+" + Math.round(n)
			: String(Math.round(n) || n);
	const pct = (n) =>
		typeof n === "number" ? (n * 100).toFixed(1) + "%" : "N/A";
		...
		.map(
			(r) =>
				...
				r
					.map(
						(v, ci) =>
							`<td class="${ci >= 4 ? "num" : ""}">${typeof v === "number" ? (ci === 1 || ci === 3 ? v : v.toLocaleString()) : esc(v)}</td>`,
					)
					.join("") +
				"</tr>",
		)
	...
	console.log(
		`Output written to ${path.join(path.dirname(holdingsFile), "output.html")}`,
	);
}
