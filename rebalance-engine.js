...
export function yieldFromPrice(cleanPrice, coupon, settleDateStr, maturityStr) {
	...
	const days = (/** @type {Date} */ a, /** @type {Date} */ b) =>
...
export function calculateGapParameters(
	...
	for (const year of [...gapYears].sort((a, b) => b - a)) {
		...
		for (const futYearStr in gapLaterMaturityInterest) {
			...
			if (futYear > year)
				sumLaterMaturityInterest += gapLaterMaturityInterest[futYear];
		}
...
export function identifyBrackets(gapYears, holdings, yearInfo) {
	...
	if (
		...
		throw new Error(
			`Could not find full brackets. Lower: ${lowerYear}, Upper: ${upperYear}`,
		);
	}
