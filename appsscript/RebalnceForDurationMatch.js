...
function rebalanceHoldingsForGapDurationMatch() {
	...
	for (const year of allYearsSorted) {
		...
		for (const holding of yearInfo[year].holdings) {
			...
			const refCPIOnDated = lookupValue(
				holding.cusip,
				tipsrefData,
				0,
				1,
			...
			const lastYearInterest =
				monthF < 7
					? adjustedAnnualInterest * 0.5
					: adjustedAnnualInterest * 1.0;
	...
	Logger.log(
		"Lower bracket: " + brackets.lowerYear + " CUSIP " + brackets.lowerCUSIP,
	);
	Logger.log(
		"Upper bracket: " + brackets.upperYear + " CUSIP " + brackets.upperCUSIP,
	);
	...
	for (const bracketYear of [brackets.lowerYear, brackets.upperYear]) {
		...
		for (const year in futureInterestByYear) {
			if (parseInt(year) > bracketYear)
				sumFutureInterest += futureInterestByYear[year];
		}
		...
		if (yearHoldings.length === 1) {
			const piPerBond = calculatePIPerBond(
				bracketCUSIP,
				bracketMaturity,
				refCPI,
				tipssaoData,
			...
			const bracketPIPerBond = calculatePIPerBond(
				bracketCUSIP,
				bracketMaturity,
				refCPI,
				tipssaoData,
	...
	for (const rebalYear of rebalanceYears) {
		...
		for (const h of yearHoldings) {
			if (h.qty > maxQty) {
				maxQty = h.qty;
				targetCUSIP = h.cusip;
				targetMaturity = h.maturity;
			}
		}

		let sumFutureInterest = 0;
		for (const year in futureInterestByYear) {
			if (parseInt(year) > rebalYear)
				sumFutureInterest += futureInterestByYear[year];
		}
		for (const year in rebalanceAddedInterest) {
			if (parseInt(year) > rebalYear)
				sumFutureInterest += rebalanceAddedInterest[year];
		}
		...
		if (yearHoldings.length === 1) {
			...
			const targetPIPerBond = calculatePIPerBond(
				targetCUSIP,
				targetMaturity,
				refCPI,
				tipssaoData,
	...
	for (let i = holdings.length - 1; i >= 0; i--) {
		...
		for (const year in outputFutureInterest) {
			if (parseInt(year) > h.year)
				sumFutureAnnualInterest += outputFutureInterest[year];
		}
		...
		if (
			buySellTargets[h.year] &&
			h.cusip === buySellTargets[h.year].targetCUSIP
		) {
	...
	const beforeLowerWeight =
		totalCurrentExcess > 0 ? lowerCurrentExcess / totalCurrentExcess : "N/A";
	const beforeUpperWeight =
		totalCurrentExcess > 0 ? upperCurrentExcess / totalCurrentExcess : "N/A";
	...
	const afterLowerWeight =
		totalExcessCost > 0 ? lowerExcessCost / totalExcessCost : "N/A";
	const afterUpperWeight =
		totalExcessCost > 0 ? upperExcessCost / totalExcessCost : "N/A";

	const lowerCurrentQty = yearInfo[brackets.lowerYear].holdings.find(
		(h) => h.cusip === lowerCUSIP,
	);
	const upperCurrentQty = yearInfo[brackets.upperYear].holdings.find(
		(h) => h.cusip === upperCUSIP,
	);
	...
	const summaryData = [
		...
		[
			"  Excess Qty (before)",
			totalCurrentExcess > 0
				? Math.round(lowerCurrentExcess / lowerCostPerBond)
				: "N/A",
			"",
			"",
		],
		...
		[
			"  Excess Qty (before)",
			totalCurrentExcess > 0
				? Math.round(upperCurrentExcess / upperCostPerBond)
				: "N/A",
			"",
			"",
		],
	...
	outputSheet
		.getRange(1, summaryCol, summaryData.length, 4)
		.setValues(summaryData);

	Logger.log(
		"Before weights: lower=" +
			beforeLowerWeight +
			", upper=" +
			beforeUpperWeight,
	);
	Logger.log(
		"After weights: lower=" + afterLowerWeight + ", upper=" + afterUpperWeight,
	);
...
function lookupRefCPI(settlementDate, refCPIData) {
	...
	throw new Error(
		"Could not find refCPI for settlement date: " + settlementDate,
	);
}
...
function calculateGapParameters(
	gapYears,
	settlementDate,
	refCPI,
	tipssaoData,
	...
	const refCPIOnDated2040 = lookupValue(
		tips2040.cusip,
		tipsrefData,
		0,
		1,
	...
	if (!anchorBefore || !anchorAfter)
		throw new Error("Could not find interpolation anchors");
	...
	for (const year of sortedGapYears) {
		...
		const syntheticYield =
			anchorBefore.yield +
			((syntheticMat - anchorBefore.maturity) *
		...
		for (const futYear in gapFutureInterest) {
			if (parseInt(futYear) > year)
				sumFutureInterest += gapFutureInterest[futYear];
		}
...
function identifyBrackets(gapYears, holdings, yearInfo) {
	...
	for (const h of holdings) {
		if (
			h.year >= LOWEST_LOWER_BRACKET_YEAR &&
			h.year < minGapYear &&
			h.qty > maxQty
		) {
			maxQty = h.qty;
	...
	if (!lowerYear) {
		throw new Error(
			"Could not find lower bracket maturity between " +
...
function calculatePIPerBond(cusip, maturity, refCPI, tipssaoData, tipsrefData) {
	...
	const lastYearInterest =
		monthF < 7 ? adjustedAnnualInterest * 0.5 : adjustedAnnualInterest * 1.0;
