...
function parseArgs() {
	...
	for (let i = 0; i < args.length; i++) {
		...
		if (args[i] === "--cash") {
			cash = parseFloat(args[++i]);
			continue;
		}
		if (args[i] === "--start-year") {
...
async function promptInputs(
	...
	const modeAns = (
		await ask(
			"Are you (R)ebalancing an existing ladder or building a (N)ew one? [default: R]: ",
		)
	)
		.trim()
		.toLowerCase();
	...
	if (isNewBuild) {
		...
		if (!startYear)
			startYear = parseInt(
				(
					await ask(`Start year [default: ${new Date().getFullYear()}]: `)
				).trim() || new Date().getFullYear().toString(),
				10,
			);
		if (!endYear)
			endYear = parseInt(
				(await ask(`End year [default: ${startYear + 10}]: `)).trim() ||
					(startYear + 10).toString(),
				10,
			);
		if (!cash)
			cash = parseFloat((await ask("Initial cash to invest: ")).trim() || "0");
		if (!dara)
			dara = parseFloat(
				(await ask("Target annual real income (DARA): ")).trim() || "0",
			);
		holdingsFile = null;
	} else {
		if (!holdingsFile) {
...
async function main() {
	...
	if (process.argv.length <= 2) {
		({ holdingsFile, dara, method, cash, startYear, endYear } =
			await promptInputs(holdingsFile, dara, method, cash, startYear, endYear));
	}
