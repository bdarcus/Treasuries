...
async function main() {
	...
	const refRows = fs
		...
		.filter((l) => l.trim())
		.map((line) => {
			const [cusip, maturity, datedDate, coupon, baseCpi, term] =
				line.split(",");
			return {
				cusip,
				maturity,
	...
	if (priceRows.length === 0)
		throw new Error("No TIPS price data found from FedInvest");
	...
	for (const p of priceRows) {
		...
		const yld = price
			? yieldFromPrice(price, ref.coupon, settleDateStr, ref.maturity)
			: null;
